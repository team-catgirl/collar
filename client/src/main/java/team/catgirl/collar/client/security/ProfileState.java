...
public final class ProfileState {
    ...
    public void write(HomeDirectory home) {
        try {
            Utils.createJsonMapper().writeValue(new File(home.profile(), "profile.json"), this);
        } catch (IOException e) {
    ...
    public static ProfileState read(HomeDirectory home)  {
        try {
            return Utils.createJsonMapper().readValue(new File(home.profile(), "profile.json"), ProfileState.class);
        } catch (IOException e) {
            throw new IllegalStateException(e);
        }
    }
